/* Ordinal Numbers: Ordinal numbers indicate their position in a array,
 such as 1st or 2nd. Most ordinal numbers end in th, except 1, 2, and 3.

• Store the numbers 1 through 9 in a array.

• Loop through the array.

• Use an if-else chain inside the loop to print the proper ordinal ending for each number. 
Your output should read "1st 2nd 3rd 4th 5th 6th 7th 8th 9th", and each result should be on a separate line. */

var numbers:number[] = [1,2,3,4,5,6,7,8,9]

numbers.forEach(ordinalNumbers)

function ordinalNumbers(numbers){
    if(numbers == 1) {
        var n = numbers.toString()
        var on = `${n}st`
        return console.log(on);
    } else if (numbers == 2) {
        var n = numbers.toString()
        var on = `${n}nd`
        return console.log(on);
    } else if (numbers == 3) {
        var n = numbers.toString()
        var on = `${n}rd`
        return console.log(on);
    } else {
        var n = numbers.toString()
        var on = `${n}th`
        return console.log(on);
    }
}
// LOOPS THROUGH EACH NUMBER, THEN CONVERTS IT INTO STRING, THEN ADDS THE ORDINAL THINGY